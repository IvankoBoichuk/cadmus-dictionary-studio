import { useEffect, useState } from "react";

import { API, apiMessageFrom } from "../api";

export type VerificationState =
  | { kind: "loading"; message: string }
  | { kind: "success"; message: string }
  | { kind: "error"; message: string };

const LOADING_STATE: VerificationState = {
  kind: "loading",
  message: "Підтверджуємо email…",
};
const MISSING_TOKEN_STATE: VerificationState = {
  kind: "error",
  message: "У посиланні немає токена підтвердження.",
};

function failureState(error: unknown): VerificationState {
  return {
    kind: "error",
    message:
      apiMessageFrom(error) ??
      "Сервіс підтвердження недоступний. Спробуйте пізніше.",
  };
}

export function useEmailVerification(token: string | null): VerificationState {
  const [resolution, setResolution] = useState<{
    token: string;
    state: VerificationState;
  } | null>(null);

  useEffect(() => {
    if (!token) return;

    let ignore = false;
    void API.auth.verifyEmail({ token }).then(
      ({ message }) => {
        if (!ignore) {
          setResolution({
            token,
            state: {
              kind: "success",
              message: message ?? "Email підтверджено. Акаунт активовано.",
            },
          });
        }
      },
      (error: unknown) => {
        if (!ignore) setResolution({ token, state: failureState(error) });
      },
    );

    return () => {
      ignore = true;
    };
  }, [token]);

  if (!token) return MISSING_TOKEN_STATE;
  return resolution?.token === token ? resolution.state : LOADING_STATE;
}
