...
import type { ImportCommitResponse, ImportPreviewResponse } from "../api";
import { useAbbreviationImport } from "./useAbbreviationImport";
...
function csvFile(): File {
  return new File(["abbreviation,category\n"], "abbreviations.csv", {
    type: "text/csv",
  });
}
...
describe("useAbbreviationImport", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    FakeXhr.instances = [];
  });

  it("previews a file and reports valid/invalid rows (AC5)", async () => {
    vi.stubGlobal("XMLHttpRequest", FakeXhr);
    const { result } = renderHook(() => useAbbreviationImport(DICTIONARY_ID, vi.fn()));
  ...
  it("commits only the valid rows and reports the outcome (AC6)", async () => {
    const outcome: ImportCommitResponse = {
      imported: [
        {
      ...
      skipped: [{ row_number: 2, errors: { abbreviation: "Дублікат." } }],
    };
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(200, outcome));
    vi.stubGlobal("fetch", fetchMock);
    const onImported = vi.fn();
    const { result } = renderHook(() => useAbbreviationImport(DICTIONARY_ID, onImported));
